...
import { RulemakingVelocityChart } from "@/components/charts/rulemaking-velocity-chart";
import { DependencyGraph } from "@/components/charts/dependency-graph";
import { ComplianceImpactRadar } from "@/components/charts/compliance-impact-radar";
import { CrossReferenceCard } from "@/components/insights/cross-reference-card";
    ...
    <div className="min-h-screen bg-background text-text-primary lg:flex">
      ...
      <div className="flex min-h-screen flex-1 flex-col">
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          <RulemakingVelocityChart data={velocityQuery.data?.data} isLoading={velocityQuery.isLoading} error={velocityQuery.error?.message ?? null} onRetry={() => velocityQuery.refetch()} />
          <ComplianceScoreCard data={complianceQuery.data?.data} isLoading={complianceQuery.isLoading} error={complianceQuery.error?.message ?? null} onRetry={() => complianceQuery.refetch()} />
        </div>

        <div className="grid gap-6 xl:grid-cols-2">
          <DependencyGraph data={dependencyQuery.data?.data} isLoading={dependencyQuery.isLoading} error={dependencyQuery.error?.message ?? null} onRetry={() => dependencyQuery.refetch()} />
          <ComplianceImpactRadar data={complianceQuery.data?.data} isLoading={complianceQuery.isLoading} error={complianceQuery.error?.message ?? null} onRetry={() => complianceQuery.refetch()} />
        </div>

        <CrossReferenceCard data={crossReferenceQuery.data?.data} isLoading={crossReferenceQuery.isLoading} error={crossReferenceQuery.error?.message ?? null} onRetry={() => crossReferenceQuery.refetch()} />
