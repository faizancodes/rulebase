      ...
      <section className="mx-auto flex min-h-screen w-full max-w-7xl flex-col justify-center px-6 py-16 lg:px-10">
        <div className="max-w-4xl space-y-8">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Link href="/search" className="inline-flex items-center justify-center rounded-md border border-border-default bg-surface-2 px-4 py-2 text-sm font-bold text-text-primary hover:border-border-hover hover:bg-surface-3">
              Search sources
            </Link>
          </div>
        </div>
        ...
        <div className="mt-16 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {STATS.map((stat) => (
            <Card key={stat.label}>
              ...
              <p className="mt-3 text-3xl font-light text-text-primary">{formatNumber(stat.value)}</p>
            </Card>
          ))}
        </div>
